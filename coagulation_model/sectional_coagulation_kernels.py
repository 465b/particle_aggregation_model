...
class SectionalCoagulationKernels:
    ...
    with a modification regarding the normalisation terms found in Burds matlab model.
        ...
        for ll in range(2,len(radii)):
            ...
            for ii in range(1,ll): # range goes up to ll-1
                ...
                result, error = dblquad(self.symetric_integrand, 
                                        volumini[ii-1], 
                                        volumini[ii], 
                                        lambda x: np.max([volumini[jj] - x, volumini[jj-1]]),
                                        volumini[jj])

                result = self.stickiness * ( (volumini[ii-1]+volumini[ll-1]) /volumini[ii-1] /volumini[ll-1]) * result
    ...
    def sectional_kernel_2_eval(self):
        ...
        for ll in range(2,len(radii)):
            for ii in range(1,ll): # range goes up to ll-1
                ...
                result = (self.stickiness /volumini[ii-1]) * result
...
class SectionalCoagulationKernels_JL:
    """
    This class calculates the "sectional coagulation kernels" (\beta_{ijl})
    as presented in the Jackson and Lochman (J&L) (10.4319/lo.1992.37.1.0077)
